...
import { LoginModal, PurchaseModal } from './components/Modals';
...
function App() {
  ...
  // Manage tickets globally to allow adding from footer
  const [tickets, setTickets] = useState<TicketData[]>(
    Array.from({ length: 8 }).map(() => ({
      id: Math.random().toString(36).substr(2, 9).toUpperCase(),
      eventName: 'Manifesto Musical - Goiânia',
      date: '02/05/2026 às 20h00',
    }))
  );
  ...
  const handleAddTickets = (quantity: number, sector: string) => {
  ...
  const handleTransferTicket = (id: string) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans text-gray-900">
      ...
      <Footer onSocialClick={openPurchaseModal} />
